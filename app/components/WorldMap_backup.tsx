    ...
    <div className="bg-white p-6 rounded-xl shadow-lg">
      <div className="mb-4">
        ...
        <p className="text-gray-600 text-sm">
          {language === 'vn' 
            ? 'Hiển thị đầy đủ 23 văn phòng Nhật Bản và 30 quốc gia/vùng lãnh thổ như thống kê bên trái - click để xem chi tiết'
            : '左側統計と一致：日本23拠点・30カ国地域の全オフィスを表示 - クリックで詳細確認'
          }
        </p>
      </div>
      ...
      <div className="mb-4 flex flex-wrap gap-4 text-sm">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-gray-700">
            {language === 'vn' ? 'Văn phòng Nhật Bản (23)' : '日本国内拠点 (23)'}
          </span>
        </div>
        <div className="flex items-center space-x-2">
          <div className="w-4 h-4 rounded-full bg-red-500"></div>
          <span className="text-gray-700">
            {language === 'vn' ? 'Văn phòng quốc tế (30)' : '海外拠点 (30)'}
          </span>
        </div>
        <div className="flex items-center space-x-2">
          <div className="w-4 h-1 bg-gray-400"></div>
          <span className="text-gray-700">
            {language === 'vn' ? 'Kết nối từ Tokyo' : '東京からの接続'}
          </span>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className={`text-lg font-bold ${
            ...
            {cities.filter(c => c.type === 'japan').length}
          </div>
          <div className="text-xs text-gray-600">
            {language === 'vn' ? 'Văn phòng JP' : '日本拠点'}
          </div>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className="text-lg font-bold text-red-600">
            {cities.filter(c => c.type === 'world').length}
          </div>
          <div className="text-xs text-gray-600">
            {language === 'vn' ? 'Quốc gia/vùng' : '国・地域'}
          </div>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className={`text-lg font-bold ${
      ...
      <div className="mt-3 p-3 bg-green-50 rounded-lg">
        <p className="text-xs text-green-700">
          {language === 'vn' 
            ? '✅ Số liệu hoàn toàn khớp: Bản đồ hiển thị chính xác 23 văn phòng Nhật Bản + 30 quốc gia như thống kê GlobalNetwork'
            : '✅ 完全一致：地図表示はGlobalNetwork統計と完全に一致（日本23拠点・30カ国）'
          }
        </p>
      </div>
    </div>
